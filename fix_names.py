...
# Read rename config: [["OldName", "NewName", "first_field", "prefix_"], ...]
...
for lib_path in lib_files:
    ...
    for old_name, new_name, first_field, prefix in renames:
        ...
        in_test = False
        for line in lines:
            if f"fn test_{prefix}" in line:
                in_test = True
            elif in_test and line.strip().startswith("fn test_") and f"test_{prefix}" not in line:
                in_test = False
