...
# Build prefix key for module name (use first prefix)
...
""")
    ...
    # Build test blocks inside a new module
    test_fns = []
    for prefix, info in prefixes.items():
        raw = prefix.rstrip("_")
        struct_name = info[0]
...
"""
    ...
    # Find the first #[cfg(test)] to insert types before it
    # Use the LAST occurrence of #[cfg(test)] that starts a major test section
    lines = content.split("\n")
    ...
    if first_test_idx is not None:
        # Insert types before the first test module
        lines.insert(first_test_idx, new_types)
        # Append test module at end
        ...
        # No test modules, append both at end
        lines.append("")
